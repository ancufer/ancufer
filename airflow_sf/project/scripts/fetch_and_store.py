...
def get_weather_data(lat, lon, city_name):
    ...
    # дополнительный эвристический индекс комфортности
    df_daily = df_daily.with_columns(
        (
...
# --- НОВЫЕ ФУНКЦИИ, КОТОРЫЕ ИМПОРТИРУЮТСЯ В DAG ---
def fetch_historical():
    ...
    if all_data:
        ...
        full_df = full_df.with_columns(pl.lit("Прошлые данные с сайта open-meteo.com").alias("Данные"))
        save_to_db(full_df)
        deduplicate_db()
...
def fetch_forecast():
    ...
    if all_data:
        full_df = pl.concat(all_data)
        full_df = full_df.with_columns(pl.lit("Прогноз с сайта open-meteo.com").alias("Данные"))
        save_to_db(full_df)
        deduplicate_db()
...
# Здесь в main нет необходимости (нужна для проверки кода)
# def main():
